  ...
  module V1
    ...
    class ThemePointsController < Api::BaseController
      ...
      def save
        ...
        ThemePoint.where(group_id: group_id, voter_id: voter_id).destroy_all
        params[:data][:vote_points].each do |alloc|
          ThemePoint.create(group_id: group_id, voter_id: voter_id, theme_id: alloc[:theme_id], points: alloc[:points])
        end
        render json: 'ok'
      end
